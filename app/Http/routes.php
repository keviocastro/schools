...
Route::group(['middleware' => 'response.cache'], function(){
...
$api->version('v1', function () use ($api) {
	...
	$api->group(['middleware' => 'config'], function() use ($api){

		$api->group(['middleware' => 'cors'], function () use ($api) {
			
			$api->group(['middleware' => 'auth0.jwt'], function () use ($api) {
	        
				$api->group(['middleware' => 'response.cache'], function() use ($api) {
